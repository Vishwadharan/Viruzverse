...
import {
  ...
  QrCode, MonitorCheck, Tag, Receipt, Wifi,
} from "lucide-react";
...
/* ── GoSip mock modules ── */
const GS_MODULES = [
  { icon: QrCode,       label: "Digital QR Menu",        sub: "Contactless · Instant update"     },
  { icon: MonitorCheck, label: "Kitchen Display (KDS)",  sub: "Live routing · Zero paper"        },
  { icon: BarChart3,    label: "Revenue Analytics",      sub: "Daily · Item-level · Trends"      },
  { icon: Tag,          label: "Offers & Discounts",     sub: "Item-level · Order-level"         },
  { icon: Receipt,      label: "Billing & Invoicing",    sub: "GST-ready · Auto-generated"       },
];

/* ── CafeAura dashboard mockup ── */
function CafeAuraMockup() {
  const COLOR = VIOLET;
  return (
    <motion.div
    ...
    >
      ...
      <div className="p-5 flex flex-col gap-3">
        {CA_MODULES.map(({ icon: Icon, label, sub }, i) => (
          <motion.div key={label}
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] font-mono mt-0.5 truncate" style={{ color: "rgba(255,255,255,0.3)" }}>{sub}</p>
            </div>
            <div className="flex items-center gap-1 shrink-0">
              {i === 0 ? (
                ...
                [...Array(3)].map((_, j) => (
                  <motion.div key={j} className="w-1 h-1 rounded-full" style={{ background: COLOR }}
        ...
        <div className="flex items-center justify-between pt-1"
          ...
          <div className="flex items-center gap-1">
            {[...Array(3)].map((_, i) => (
              <motion.div key={i} className="w-1 h-1 rounded-full" style={{ background: "#2e1f4a" }}
                animate={{ opacity: [0.3, 1, 0.3] }}
...
function GoSipMockup() {
  return (
    <motion.div
      ...
      style={{
        background: "#080f09",
        boxShadow: `0 0 0 1px ${GREEN}20, 0 40px 100px rgba(22,163,74,0.15)`,
      }}
    >
      {/* Chrome */}
      <div className="flex items-center justify-between px-5 py-3.5"
        style={{ background: "#0a120b", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
        <div className="flex items-center gap-1.5">
          {["#ff5f57","#febc2e","#28c840"].map((c, i) => (
            <div key={i} className="w-2.5 h-2.5 rounded-full" style={{ background: c }} />
          ))}
        </div>
        <span className="text-[11px] font-mono" style={{ color: "#14532d" }}>gosip-food.in</span>
        <div className="flex items-center gap-1.5">
          <span className="relative flex h-2 w-2">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full opacity-75"
              style={{ background: GREEN2 }} />
            <span className="relative flex h-2 w-2 rounded-full" style={{ background: GREEN }} />
          </span>
          <span className="text-[10px] font-mono" style={{ color: GREEN }}>live</span>
        </div>
      </div>
      ...
      <div className="p-5 flex flex-col gap-3">
        ...
        <div className="flex items-center justify-between px-3 py-2 rounded-xl mb-1"
          ...
          <div className="flex items-center gap-2">
            <QrCode size={13} style={{ color: GREEN }} strokeWidth={1.7} />
            <span className="text-[11px] font-mono font-semibold" style={{ color: GREEN }}>
              TABLE 3 · Brew Corner
            </span>
          </div>
          <span className="text-[9px] font-mono px-2 py-0.5 rounded-full"
            style={{ background: `${GREEN}15`, color: GREEN2 }}>
            2 orders today
          </span>
        </div>

        {GS_MODULES.map(({ icon: Icon, label, sub }, i) => (
          <motion.div key={label}
            initial={{ opacity: 0, x: -12 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            ...
            className="flex items-center gap-3 px-3 py-2.5 rounded-2xl"
            style={{
              background: i === 0 ? `${GREEN}0d` : "rgba(255,255,255,0.03)",
              border:     i === 0 ? `1px solid ${GREEN}22` : "1px solid rgba(255,255,255,0.05)",
            }}>
            <div className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0"
              style={{ background: `${GREEN}18`, border: `1px solid ${GREEN}25` }}>
              <Icon size={14} style={{ color: GREEN2 }} strokeWidth={1.7} />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-[12px] font-bold text-white leading-tight">{label}</p>
              <p className="text-[10px] font-mono mt-0.5 truncate" style={{ color: "rgba(255,255,255,0.3)" }}>{sub}</p>
            ...
            <div className="flex items-center gap-1 shrink-0">
              {i === 0 ? (
                ...
                [...Array(3)].map((_, j) => (
                  <motion.div key={j} className="w-1 h-1 rounded-full" style={{ background: GREEN }}
                    animate={{ opacity: [0.3, 1, 0.3] }}
                    transition={{ duration: 1, repeat: Infinity, delay: j * 0.2, ease: "easeInOut" }} />
                ))
              )}
        ...
        <div className="flex items-center justify-between pt-1"
          style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }}>
          <span className="text-[10px] font-mono" style={{ color: "#14532d" }}>
            React · Supabase · AWS
          </span>
          <div className="flex items-center gap-1">
            {[...Array(3)].map((_, i) => (
              <motion.div key={i} className="w-1 h-1 rounded-full" style={{ background: "#14532d" }}
                animate={{ opacity: [0.3, 1, 0.3] }}
                transition={{ duration: 1.2, repeat: Infinity, delay: i * 0.2, ease: "easeInOut" }} />
            ))}
          </div>
...
export default function ProductsPage() {
  return (
    <main className="min-h-screen" style={{ background: "#07060f" }}>
      ...
      {/* ══ HERO ══ */}
      <section className="relative pt-25 pb-28 overflow-hidden">
        <div className="absolute inset-0 pointer-events-none"
          style={{
            backgroundImage:
              `linear-gradient(${VIOLET}06 1px,transparent 1px),` +
              `linear-gradient(90deg,${VIOLET}06 1px,transparent 1px)`,
            backgroundSize: "52px 52px",
          }} />
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[700px] h-56 pointer-events-none"
          style={{ background: `radial-gradient(ellipse,${VIOLET}22 0%,transparent 70%)` }} />
        ...
        <div className="absolute bottom-0 inset-x-0 h-px"
          style={{ background: `linear-gradient(90deg,transparent,#c4b5fd 40%,${VIOLET} 50%,#c4b5fd 60%,transparent)` }} />
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-5 sm:px-8 text-center">
          <motion.div {...rise(0)} className="flex justify-center mb-8">
            <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-[11px] font-mono font-semibold"
              style={{ color: "#a78bfa", background: `${VIOLET}10`, border: `1px solid ${VIOLET}28` }}>
      ...
      <section className="relative pb-24 overflow-hidden">
        <div className="absolute inset-0 pointer-events-none"
          style={{
            backgroundImage:
              `linear-gradient(${VIOLET}04 1px,transparent 1px),` +
              `linear-gradient(90deg,${VIOLET}04 1px,transparent 1px)`,
            backgroundSize: "52px 52px",
          }} />
        <div className="absolute bottom-0 inset-x-0 h-px"
          style={{ background: `linear-gradient(90deg,transparent,#c4b5fd 40%,${VIOLET} 50%,#c4b5fd 60%,transparent)` }} />
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-5 sm:px-8">
          <motion.div {...rise(0)}
            ...
            <div className="h-[2px] w-full"
              style={{ background: `linear-gradient(90deg,${VIOLET},#a78bfa,#6366f1)` }} />
              ...
              <div>
                <motion.div {...rise(0.06)} className="flex items-center gap-3 flex-wrap mb-6">
                  <span className="px-3 py-1 rounded-full text-[11px] font-mono font-semibold"
                    style={{ background: `${VIOLET}18`, color: "#a78bfa", border: `1px solid ${VIOLET}28` }}>
                    SaaS Platform
                  </span>
                ...
                <motion.h2 {...rise(0.1)}
                  ...
                  style={{
                    ...
                    backgroundImage: `linear-gradient(118deg,${VIOLET} 10%,#a78bfa 55%,#c4b5fd)`,
                    WebkitBackgroundClip: "text",
                    WebkitTextFillColor: "transparent",
                    backgroundClip: "text",
                  }}>
                  ...
                  {[
                    ...
                    <div key={item} className="flex items-start gap-2.5">
                      ...
                      <span className="text-[13px] leading-[1.6]"
                        style={{ color: "rgba(255,255,255,0.55)" }}>{item}</span>
                    </div>
                  ))}
                </motion.div>

                <motion.div {...rise(0.24)} className="flex flex-wrap gap-3">
                  <Link href="/products/cafeaura"
                    ...
                    style={{
                      ...
                      boxShadow: `0 8px 28px ${VIOLET}45, inset 0 1px 0 rgba(255,255,255,0.15)`,
                    }}>
                    Explore CafeAura
      ...
      <section className="relative pb-24 overflow-hidden">
        <div className="absolute inset-0 pointer-events-none"
          style={{
            backgroundImage:
              `linear-gradient(${GREEN}04 1px,transparent 1px),` +
              `linear-gradient(90deg,${GREEN}04 1px,transparent 1px)`,
            backgroundSize: "52px 52px",
          }} />
        <div className="absolute top-0 right-0 w-[600px] h-[400px] pointer-events-none"
          style={{ background: `radial-gradient(ellipse at 100% 0%,${GREEN}0a,transparent 65%)` }} />
        <div className="absolute bottom-0 inset-x-0 h-px"
          style={{ background: `linear-gradient(90deg,transparent,${GREEN2}60 40%,${GREEN} 50%,${GREEN2}60 60%,transparent)` }} />
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-5 sm:px-8">
          <motion.div {...rise(0)}
            ...
            {/* Green top bar */}
            <div className="h-[2px] w-full"
              style={{ background: `linear-gradient(90deg,${GREEN},${GREEN2},#bbf7d0)` }} />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-12 p-8 sm:p-12">
              ...
              <div>
                <motion.div {...rise(0.06)} className="flex items-center gap-3 flex-wrap mb-6">
                  <span className="px-3 py-1 rounded-full text-[11px] font-mono font-semibold"
                    style={{ background: `${GREEN}18`, color: GREEN2, border: `1px solid ${GREEN}28` }}>
                    Restaurant SaaS
                  </span>
                  <span className="flex items-center gap-1.5 text-[11px] font-mono" style={{ color: "#10b981" }}>
                    <span className="relative flex h-2 w-2">
                ...
                <motion.h2 {...rise(0.1)}
                  ...
                  style={{
                    fontSize: "clamp(36px,5.5vw,64px)",
                    backgroundImage: `linear-gradient(118deg,${GREEN} 10%,${GREEN2} 55%,#bbf7d0)`,
                    WebkitBackgroundClip: "text",
                    WebkitTextFillColor: "transparent",
                    backgroundClip: "text",
                  }}>
                ...
                <motion.p {...rise(0.16)} className="text-[14px] leading-[1.85] mb-10 max-w-lg"
                  ...
                  Digital menu, kitchen display, revenue analytics, and GST billing — unified
                  in one platform for every F&amp;B outlet.
                </motion.p>

                <motion.div {...rise(0.2)} className="flex flex-col gap-3 mb-10">
                  {[
                    "Contactless digital menu via table QR codes",
                    "Kitchen Display System (KDS) — orders routed in real time",
                    "Revenue analytics with item-level insights",
                    "Offer and discount management from the admin panel",
                    "GST-ready billing and auto-invoicing (Pro plan)",
                  ].map(item => (
                    <div key={item} className="flex items-start gap-2.5">
                      <CheckCircle2 size={14} style={{ color: GREEN2 }} className="mt-0.5 shrink-0" />
                      <span className="text-[13px] leading-[1.6]"
                        style={{ color: "rgba(255,255,255,0.55)" }}>{item}</span>
                    </div>
                  ))}
                ...
                <motion.div {...rise(0.24)} className="flex flex-wrap gap-3">
                  <Link href="/products/gosip"
                    ...
                    style={{
                      ...
                      boxShadow: `0 8px 28px ${GREEN}45, inset 0 1px 0 rgba(255,255,255,0.15)`,
                    }}>
                    Explore GoSip
                    <ArrowUpRight size={15}
                      className="transition-transform duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
        ...
        <div className="max-w-6xl mx-auto px-5 sm:px-8">
          <motion.div {...rise(0)}
            ...
            <div className="h-[2px] w-full"
              style={{ background: `linear-gradient(90deg,${VIOLET},#a78bfa,#6366f1)` }} />
            <div className="absolute inset-0 pointer-events-none"
              style={{ background: `radial-gradient(ellipse at 20% 50%,${VIOLET}0c 0%,transparent 60%)` }} />
            ...
            <div className="relative z-10 flex flex-col lg:flex-row items-start lg:items-center
              ...
              <div className="max-w-lg">
                <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full mb-5"
                  style={{ background: `${VIOLET}0f`, border: `1px solid ${VIOLET}25` }}>
                ...
                <h2 className="font-black tracking-[-0.03em] leading-[1.06] mb-3 text-white"
                  style={{ fontSize: "clamp(22px,3vw,38px)" }}>
                  Have a product
                  <br />
                  <span style={{
                    backgroundImage: `linear-gradient(118deg,${VIOLET} 10%,#a78bfa 70%)`,
                    WebkitBackgroundClip: "text",
                    WebkitTextFillColor: "transparent",
                    backgroundClip: "text",
                  }}>
              ...
              <div className="flex flex-col sm:flex-row gap-3 shrink-0">
                <Link href="/contact"
                  ...
                  style={{
                    background: `linear-gradient(135deg,${VIOLET},#6d28d9)`,
                    boxShadow: `0 8px 24px ${VIOLET}45, inset 0 1px 0 rgba(255,255,255,0.15)`,
                  }}>
                  Start a project
